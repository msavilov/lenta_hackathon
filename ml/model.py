...
holydays = pd.read_csv('F:\\holidays_covid_calendar.csv') #файл с праздниками

# Load the model
model = RandomForestRegressor()
model.load_model('trained_model.joblib')
model_pr = Prophet()
model_pr.load_model('trained_model_pr.joblib')
...
def train(train, test):
    """
    Здесь мы формируем дополнительные признаки и добавляем колонки, по которым отделим трейн и тест,
    чтоб подать в модель только тест из запроса
    """
    train['num'] = range(len(train))
    test['num'] = range(len(test_s))
    ...
    rub = train[train['pr_sales_type_id']==0].\
    pivot_table(index='pr_sku_id', values='pr_sales_in_rub_id', aggfunc='last')
    
    test = test.merge(rub, on='pr_sku_id', how='left').set_index(test.index)
    
    train = train.append(test)
    train = train[train['st_is_active'] == 1]
    train['pr_sales_in_rub_id'] = round(train['pr_sales_in_rub'] / train['pr_sales_in_units'], 1)
...
def make_features(df_mf, max_lag, rolling_mean_size):
    ...
    for s in df_mf['pr_sales_type_id'].unique():
        ...
        df_mf_temp['sin_dayofweek'] = np.sin(df_mf_temp['dayofweek'] * np.pi * 2 / np.max(df_mf_temp['dayofweek']))
        df_mf_temp['sin_month'] = np.sin(df_mf_temp['month'] * np.pi * 2 / np.max(df_mf_temp['month']))
        df_mf_temp['sin_day'] = np.sin(df_mf_temp['day'] * np.pi * 2 / np.max(df_mf_temp['day']))
        df_mf_temp['cos_dayofweek'] = np.cos(df_mf_temp['dayofweek'] * np.pi * 2 / np.max(df_mf_temp['dayofweek']))
        df_mf_temp['cos_month'] = np.cos(df_mf_temp['month'] * np.pi * 2 / np.max(df_mf_temp['month']))
        df_mf_temp['cos_day'] = np.cos(df_mf_temp['day'] * np.pi * 2 / np.max(df_mf_temp['day']))

        lags = list(range(14, max_lag + 1))
        for lag in lags:
            df_mf_temp['sales_lag_' + str(lag)] = df_mf_temp.groupby(["st_id", "pr_sku_id"])['pr_sales_in_units'].transform(
                lambda x: x.shift(lag))
            
        df_mf_temp['rolling_mean_units'] = df_mf_temp.groupby(["st_id", "pr_sku_id"])['pr_sales_in_units'].transform(
...
def prophet(df):
    ...
    pr_train = train_p_f.reset_index().sort_values(by=['st_id', 'pr_sku_id', 'date'])
    pr_train['st_pr_sku'] = pr_train['st_id'] + ' ' + pr_train['pr_sku_id']
    pr_train = pr_train.rename(columns={'date': 'ds', 'pr_sales_in_units': 'y'})
    
    pr_test = test_p_f.reset_index().sort_values(by=['st_id', 'pr_sku_id', 'date'])
    pr_test['st_pr_sku'] = pr_test['st_id'] + ' ' + pr_test['pr_sku_id']
    ...
    for id in pr_train['st_pr_sku'].unique():
        if len(pr_train.loc[pr_train['st_pr_sku'] == id]) > 2: #профет не предсказывает для рядов с менее чем 2-мя измерениями
            ...
            p.append(forecast[['ds', 'st_pr_sku', 'trend', 'yearly', 'weekly', 'yhat']])
        else:
...
def holydays(df, holydays_df):
    ...
    holydays_df['date'] = holydays_df['calday'].astype(str).apply(lambda x: x[:4]+'-' + x[4:6]+'-'+x[6:8])
    ...
    holydays_df.loc[(holydays_df['date'] >= '2022-12-30') & (holydays_df['date'] <= '2023-01-01'), 'new_year'] = 1
    holydays_df['easter'] = 0
    holydays_df.loc[(holydays_df['date']>='2023-04-14') & (holydays_df['date']<='2023-04-16'), 'easter'] = 1
...
def mean(train_m):
    ...
    # Создаем словарь для хранения средних значений по каждой категории в каждом признаке
    mean_encoding_dict = {}
    ...
    for feature in features_cat:
        ...
        mean_encoding_dict[feature] = train_m.groupby(feature)['pr_sales_in_units'].mean()
    ...
    for feature in features_cat:
        train_m[feature + '_mean_encoded'] = train_m[feature].map(mean_encoding_dict[feature])
    
    # Удаляем исходные категориальные признаки, так как мы их заменили закодированными значениями
...
def forecast(train_m):
    
    """Здесь данные мы уже конечные отбираем удаляем лишнее и загоняем в модель"""
    ...
    test_final = test_final.drop(['pr_sales_in_rub', 'pr_sales_in_units', 'num', 'type_df'], axis=1)
    
    predict = model.predict(test_final)
    return predict

predict = forecast(train_fin)